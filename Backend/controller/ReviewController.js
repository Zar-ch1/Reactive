...
const saveReview = async (req,res,next)=>{
    try {
        ...
        if(!review)
        {
            throw new Error('review cannot be saved');
        }
        await res.status(201).json(review);

    }catch(err)
    {
        await res.status(400).json({message: err.toString()})
    }
}
async function saveReviewA(req,res,next){
    try{
        const body = req.body;
        const review = await reviewService.saveReviewA(body);
        if(!review)
        {
            throw Error('Review cant not be saved');
        }
        await res.status(201).json(review);
    }
    catch(err)
